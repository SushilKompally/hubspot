...
#!/usr/bin/env python3
"""
Generate silver models for each hubspot source instance by reading dbt_project.yml
and copying/replacing tokens in reusable silver templates.

Features:
- Robust parsing of vars.hubspot.source_instances supporting multiple YAML shapes
  (list of strings, dict of instances, and list of ad-hoc dicts like in user's example).
- Creates per-instance silver models with unique filenames (e.g., account__hubspot1.sql)
...
Usage:
    python generate_silver_models.py \
        ...
        --templates-dir hubspot/silver \
        --output-dir models/hubspot/silver \
...
def _normalize_instances(si: Any) -> List[Dict[str, Any]]:
    """
    Normalizes vars.hubspot.source_instances into a list like:
        [{"name": "hubspot1", ...}, {"name": "hubspot2", ...}]
    Supports:
      1) list of strings: ['hubspot1', 'hubspot2']
      2) dict: {'hubspot1': {...}, 'hubspot2': {...}}
      3) list of single-key dicts whose value is dict OR list-of-dicts:
         - hubspot1:
           - source_database_name: hubspot_db
           - source_schema_name: hubspot1
           - tool_name: fivetran
    """
    ...
    if isinstance(si, list):
        for item in si:
            if isinstance(item, str):
                ...
                if "name" in item:
                    ...
                    # shape: {hubspot1: <dict or list of dicts>}
                    k = next(iter(item))
                    v = item[k]
                    inst = {"name": str(k)}
        ...
        raise ValueError("vars.hubspot.source_instances must be a list or a dict.")
...
def extract_hubspot_config(project: Dict[str, Any]) -> Dict[str, Any]:
    vars_dict = project.get("vars", {}) or {}
    hubspot = vars_dict.get("hubspot", {}) or {}

    defaults = {
        "source_database_name": hubspot.get("source_database_name") or vars_dict.get("source_database_name"),
        "source_schema_prefix": hubspot.get("source_schema_prefix") or vars_dict.get("source_schema_prefix"),
    }

    # Look for source_instances in hubspot first, then in vars root
    si = hubspot.get("source_instances") or vars_dict.get("source_instances")
...
def substitute_instance_token(text: str, instance_name: str) -> str:
    ...
    replaced = re.sub(r"hubspot__\s*__INSTANCE__", f"hubspot__{instance_name}", replaced)
    return replaced
...
def copy_and_render_templates_per_instance(
    ...
    for tpl in files:
        ...
        # e.g., account.sql.tpl -> account__hubspot1.sql
...
def main():
    ...
    cfg = extract_hubspot_config(project)

    instances = cfg["source_instances"]
    if not instances:
        raise RuntimeError("No source instances found in dbt_project.yml under vars.hubspot.source_instances.")
